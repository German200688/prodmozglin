...
static int directorD(char* prodmotorD) {
    std::cout << "prodmotor = " << prodmotorD << std::endl;
    LOG_TRACE("prodmotor = prodmotor{}", prodmotorD);
    rd_kafka_t* producerD;
    ...
    // Fixed properties
    set_configD(confD, "acks", "all");

    // Install a delivery-error callback.
    rd_kafka_conf_set_dr_msg_cb(confD, dr_msg_cb);

    // Create the Producer instance.
    producerD = rd_kafka_new(RD_KAFKA_PRODUCER, confD, errstrD, sizeof(errstrD));
    if (!producerD) {
        std::cout << "Failed to create new producer: " << errstrD << std::endl;
        LOG_ERROR("Failed to create new producer: errstr{}", errstrD);
        return 1;
    }
    ...
    const char* topic = "prodmozgd";
    const char* user_ids[1] = { "commandpotom" };
    const char* products[1] = { prodmotorD };
    std::cout << "prodmotor after = " << prodmotorD << std::endl;
    LOG_TRACE("prodmotor after =  prodmotor{}", prodmotorD);
    ...
    for (int i = 0; i < message_count; i++) {
        ...
        err = rd_kafka_producev(producerD,
            RD_KAFKA_V_TOPIC(topic),
            RD_KAFKA_V_MSGFLAGS(RD_KAFKA_MSG_F_COPY),
            RD_KAFKA_V_KEY((void*)key, key_len),
            RD_KAFKA_V_VALUE((void*)value, value_len),
        ...
        rd_kafka_poll(producerD, 0);
    }

    // Block until the messages are all sent.
    std::cout << "Flushing final messages.." << std::endl;
    LOG_TRACE("Flushing final messages..");
    rd_kafka_flush(producerD, 10 * 1000);

    if (rd_kafka_outq_len(producerD) > 0) {
        std::cout << rd_kafka_outq_len(producerD) << " message(s) were not delivered" << std::endl;
        LOG_INFO("rd_kafka_outq_len(producer){} message(s) were not delivered", rd_kafka_outq_len(producerD));
    }

    std::cout << message_count << " events were produced to topic " << topic << std::endl;
    LOG_INFO("message_count{} events were produced to topic topic{}", message_count, topic);

    rd_kafka_destroy(producerD);
